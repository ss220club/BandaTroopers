...
/datum/unit_test/faction_music_resolver/Run()
	...
	if(get_faction_music_key_for_values(null, FACTION_UNSCN) != "UNSC")
		Fail("UNSC Navy roles must use the independent UNSC music key.", __FILE__, __LINE__)
	...
	if(!isnull(get_faction_music_key_for_values(null, FACTION_NEUTRAL)))
