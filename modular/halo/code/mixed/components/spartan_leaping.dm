...
/datum/component/leaping/proc/check_leap(direction)
	if(last_move_dir == direction && last_mousedown_time + double_tap_timing > world.time)
		...
		TIMER_COOLDOWN_START(parent, LEAP_COMPONENT_COOLDOWN, leap_cooldown)
...
/datum/component/leaping/proc/activate_leap(direction)
	...
	animate(leaper, pixel_z = leaper.pixel_z + 12, layer = max(ABOVE_MOB_LAYER - 0.03, original_layer), time = 0.3 SECONDS / 2, easing = CIRCULAR_EASING|EASE_OUT, flags = ANIMATION_END_NOW|ANIMATION_PARALLEL)
