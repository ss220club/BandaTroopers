...
/datum/species/zombie/handle_death(mob/living/carbon/human/zombie, gibbed)
	...
	if(zombie)
		...
		if(!QDELETED(head) && !(head.status & LIMB_DESTROYED))
			...
			if(zombie.client)
				zombie.play_screen_text("<span class='langchat_notification' style=text-align:center valign='top'><u>You are dead...</u></span><br>You lost your head. No reviving for you.", /atom/movable/screen/text/screen_text/command_order, rgb(155, 0, 200)) // SS220 Fonts
