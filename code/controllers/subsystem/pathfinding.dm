			...
			for(var/direction in GLOB.cardinals)
				...
				if(length(L))
					if(isxeno(current_run.agent))
						...
						var/datum/human_ai_brain/brain = null
						if(ishuman(current_run.agent))
							var/mob/living/carbon/human/human_agent = current_run.agent
							brain = human_agent.get_ai_brain()
						if(brain)
							for(var/atom/A as anything in L)
